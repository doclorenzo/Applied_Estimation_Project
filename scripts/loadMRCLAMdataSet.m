...
[subject_num, barcode_num] = textread('MRCLAM_Dataset2/Barcodes.dat', '%u %u','commentstyle','shell');
data.Barcodes = [subject_num, barcode_num];
clear subject_num barcode_num;

disp('Reading landmark groundtruth')
[subject_num, x, y, x_sd, y_sd] = textread('MRCLAM_Dataset2/Landmark_Groundtruth.dat', '%f %f %f %f %f','commentstyle','shell');
...
for i = 1:n_robots
    ...
    [time, x, y, theta] = textread(['MRCLAM_Dataset2/Robot' num2str(i) '_Groundtruth.dat'], ...
    ...
    [time, v, w] = textread(['MRCLAM_Dataset2/Robot' num2str(i) '_Odometry.dat'], ...
                            '%f %f %f','commentstyle','shell');
    data.Robot(i).Odometry = [time v w];
    clear time v w;

    disp(['Reading robot ' num2str(i) ' measurements'])
    [time, barcode_num, r, b] = textread(['MRCLAM_Dataset2/Robot' num2str(i) '_Measurement.dat'], ...
                                         '%f %f %f %f','commentstyle','shell');
